...
async function seedDatabase() {
  try {
    ...
    for (const productData of sampleProducts) {
      try {
        // Extract features from product metadata for meaningful similarity
        const imageFeatures = ImageService.extractFeaturesFromProduct(productData);
    ...
    const insertedProducts = await Product.insertMany(productsWithFeatures);
